...
total_sum = 0
for group in groups:
    passengers_answers = group.split("\n")
    print(passengers_answers)
    group_yes_answers = [
        answer
        for passenger in passengers_answers
        for answer in passenger
        if all(answer in p for p in passengers_answers)
    ]
    total_sum += len(set(group_yes_answers))
# print(groups)
# print(total_sum)

"""question:
is there a way to do a set() & set() for all items of each list below?
['abc']
['a', 'b', 'c']
['ab', 'ac']
['a', 'a', 'a', 'a']
